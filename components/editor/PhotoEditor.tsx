...
import { compositeImage, downloadBlob, fileToDataUrl } from "@/lib/canvas"
import type { Template, PixelCrop, EditorPhase } from "@/lib/types"
import {
  UploadIcon, CropIcon, CheckIcon,
  DownloadIcon, RotateCcwIcon, ZoomInIcon, ZoomOutIcon,
  ImageIcon, FileImageIcon
} from "lucide-react"
  ...
  const handleProceedToPreview = async () => {
    ...
    try {
      const [pngBlob, jpegBlob] = await Promise.all([
        compositeImage(userImageUrl, croppedAreaPixels, template, "png"),
        compositeImage(userImageUrl, croppedAreaPixels, template, "jpeg"),
      ])
        ...
        <div className="space-y-4">
          <div className="rounded-xl bg-muted/50 p-4 text-center">
            <p className="text-sm text-muted-foreground">
              Upload your photo and we&apos;ll fit it into the template for you.
              You can adjust the position and zoom before downloading.
            </p>
          </div>
      ...
      {phase === "crop" && userImageUrl && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground text-center">
            Pinch to zoom · Drag to reposition your photo
          </p>
          ...
          >
            {/* react-easy-crop handles touch gestures */}
            <Cropper
              image={userImageUrl}
              crop={crop}
              zoom={zoom}
          ...
          <div className="flex items-center gap-3 rounded-xl bg-muted/50 px-4 py-3">
            <ZoomOutIcon className="h-4 w-4 flex-shrink-0 text-muted-foreground" />
            <Slider
              value={[zoom]}
              min={1}
              max={3}
              ...
              className="flex-1"
            />
            <ZoomInIcon className="h-4 w-4 flex-shrink-0 text-muted-foreground" />
          </div>
