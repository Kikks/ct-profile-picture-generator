  ...
  return (
    <div className={className}>
      ...
      >
        ...
        {imgLoaded && (
          <>
            {/* Dark overlay outside selected area */}
            <div className="pointer-events-none absolute inset-0">
              {/* Top strip */}
              <div className="absolute left-0 right-0 top-0 bg-black/40" style={{ height: displayArea.y }} />
              {/* Bottom strip */}
              <div
                className="absolute bottom-0 left-0 right-0 bg-black/40"
                style={{ top: displayArea.y + displayArea.height }}
              />
              {/* Left strip */}
              <div
              ...
              <div
                ...
                style={{
                  top: displayArea.y,
                  left: displayArea.x + displayArea.width,
                  right: 0,
                  height: displayArea.height,
                }}
              />
      ...
      {imgLoaded && (
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-muted-foreground sm:grid-cols-4">
          <div className="rounded bg-muted px-2 py-1">
            <span className="font-medium">X:</span> {Math.round(value.x)}px
          </div>
          <div className="rounded bg-muted px-2 py-1">
            <span className="font-medium">Y:</span> {Math.round(value.y)}px
          </div>
          <div className="rounded bg-muted px-2 py-1">
            <span className="font-medium">W:</span> {Math.round(value.width)}px
          </div>
          <div className="rounded bg-muted px-2 py-1">
            <span className="font-medium">H:</span> {Math.round(value.height)}px
          </div>
        </div>
      )}
